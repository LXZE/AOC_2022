...
list_pos = fn map, target ->
	Enum.map(map, fn {idx_row, row} ->
		Enum.map(row, fn
			{idx_col, ^target} -> {idx_row, idx_col}
			_ -> nil
		end)
		|> Enum.reject(& !&1)
	end)
...
list_pos.(map, ?a)
|> Enum.map(&Graph.dijkstra(g, &1, end_pos))
|> Enum.filter(& &1)
|> Enum.sort_by(&length/1)
|> Enum.take(1) |> Enum.at(0) |> length |> Kernel.-(1)
|> IO.puts
