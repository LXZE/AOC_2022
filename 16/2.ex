...
solo_result = solve.(30) |> Enum.max_by(& &1.total) |> (& &1.total).()
results = solve.(26)
|> Enum.reduce(%{}, fn state, acc ->
	Map.update(acc, MapSet.new(state.path -- ["AA"]), state.total, &max(&1, state.total))
end)

Enum.max(
	for {path_human, res_human} <- results,
		{path_eleph, res_eleph} <- results,
		res_human + res_eleph > solo_result,
		MapSet.disjoint?(path_human, path_eleph), do: res_human + res_eleph
) |> IO.puts
